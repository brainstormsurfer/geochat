...
2. Create Router Configuration:
   ...
   - Configure routes for different paths, including components like `HomeLayout`, `Landing`, `Events`, etc.
...
- Create the `Header` Component:
  ...
  - Return a `header` element with classes `'bg-neutral py-2 text-neutral-content'`.
    ...
    - Inside the `header` element, create a `div` element with classes `'align-element flex justify-center sm:justify-end'`.
      ...
      - Inside the `div` element, create another `div` element with classes `'flex gap-x-6 justify-center helpers-center'`.
  ...
  return (
    <header className=' bg-neutral py-2 text-neutral-content '>
      <div className='align-element flex justify-center sm:justify-end '>
        {/* USER */}
        {/* LINKS */}
        <div className='flex gap-x-6 justify-center helpers-center'>
    ...
    - Inside the `nav` element, create a `div` element with class `'navbar align-element '`.
      ...
      - Inside the first `div` element, create another `div` element with class `'navbar-start'`.
        ...
        - Create a `NavLink` to `'/'` with the following attributes:
          ...
          - Class set to `'hidden lg:flex btn btn-primary text-3xl helpers-center '`.
      ...
      - Inside the third `div` element, create another `div` element with class `'navbar-end'`.
        ...
        - Create a `NavLink` to `'cart'` with the following attributes:
          ...
          - Inside the `NavLink`, create a `div` element with class `'indicator'`.
            ...
            - Create a `span` element with classes `'badge badge-sm badge-primary indicator-helper'` and text content `'8'`.
    ...
    <nav className='bg-base-200'>
      <div className='navbar align-element '>
        <div className='navbar-start'>
          ...
          <NavLink
            to='/'
            className='hidden lg:flex btn btn-primary text-3xl helpers-center '
          >
            C
          </NavLink>
          {/* DROPDOWN */}
        ...
        <div className='navbar-end'>
          ...
          <NavLink to='cart' className='btn btn-ghost btn-circle btn-md ml-4'>
            <div className='indicator'>
              ...
              <span className='badge badge-sm badge-primary indicator-helper'>
                8
              </span>
            </div>
          </NavLink>
...
- About Component:
  - Define the `About` component.
    - Return a fragment containing the following elements:
      - A `div` with classes `flex`, `flex-wrap`, `gap-2`, `sm:gap-x-6`, `helpers-center`, and `justify-center`.
...
const About = () => {
  return (
    <>
      <div className='flex flex-wrap gap-2 sm:gap-x-6 helpers-center justify-center'>
...
- Hero Component:
  - Define the `Hero` component.
    ...
    - Return a `div` with classes `grid`, `grid-cols-1`, `lg:grid-cols-2`, `gap-24`, and `helpers-center`.
      - Inside this `div`, another `div`.
        ...
        - Following that, a `div` with class `mt-10`.
          - Inside the `div`, a `Link` component with props `to='events'` and classes `btn` and `btn-primary`, containing the text "Our Events".
      - Another `div` with classes `hidden`, `h-[28rem]`, `lg:carousel`, `carousel-center`, `p-4`, `space-x-4`, `bg-neutral`, and `rounded-box`.
        - Inside this `div`, a JavaScript map function iterating over `carouselImages`.
          - For each image, a `div` with class `carousel-helper`.
...
const Hero = () => {
  return (
    <div className=' grid grid-cols-1 lg:grid-cols-2 gap-24 helpers-center'>
      <div>
...
## Challenge (20) - Featured Events

- create FeaturedEvents, SectionTitle and EventsGrid components
- render SectionTitle and EventsGrid in FeaturedEvents
- setup SectionTitle
- in EventsGrid access and render events from loader
...
2. Create FeaturedEvents Component:

   - Define a functional component named `FeaturedEvents`.

3. Component Structure:

   - Return a `div` element with the class `pt-24`.
   - Inside the `div`, include a `SectionTitle` component with the prop `text` set to `'featured events'`.
   - Include a `EventsGrid` component.

4. Export FeaturedEvents Component:
   - Export the `FeaturedEvents` component as the default export of the module.
...
2. Create EventsGrid Component:

   - Define a functional component named `EventsGrid`.

3. Component Structure:

   - Inside the component, destructure the `events` data using `useLoaderData`.
   - Return a `div` element with the classes `pt-12 grid gap-4 md:grid-cols-2 lg:grid-cols-3`.
   - Use the `.map()` function to iterate through each `event` in the `events` array.

4. Event Card:

   - For each `event`, destructure the attributes such as `title`, `eventDate`, and `image`.
   - Create a `Link` component that has the following:
     - `key` attribute set to `event.id`.
     - `to` attribute set to `/events/${event.id}`.
     - `className` attribute with classes for styling.
   - Inside the `Link`, create a `figure` element with the class `px-4 pt-4` to hold the event image.
   ...
   - Below the `figure`, create a `div` element with the class `card-body helpers-center text-center`.
   - Inside the `div`, display the `title` using a `h2` element with classes for styling.
   - Display the `eventDate` using a `span` element with the class `text-secondary`.

5. Export EventsGrid Component:
   - Export the `EventsGrid` component as the default export of the module.

## Solution (20) - Featured Events
...
## Challenge (21) - Format eventDate

- payment providers need in smallest unit
  - in this case cents
- in utils setup a function to format eventDate
- utilize in EventsGrid

## Solution (21) - Format eventDate
...
## Challenge (22) - Single Event

- complete in multiple steps
- fetch and render single event
...
1. Import Dependencies:
   ...
   - Import `calculateDaysLeft`, `customFetch`, and `useState` from `'../utils'`.
   - Import `Link` from `'react-router-dom'`.

2. Define Loader Function:

   - Define a loader function that fetches event data based on the `params.id`.
   - Use `customFetch` to fetch the event data from `/events/${params.id}`.
   - Return an object containing the fetched event data.

3. Create SingleEvent Component:

   - Define a functional component named `SingleEvent`.

4. Component Structure:

   - Inside the component, destructure the `event` data using `useLoaderData`.
   - Destructure attributes like `image`, `title`, `eventDate`, `description`, `colors`, and `company`.
   - Create a `countdown` variable by formatting the `eventDate` using `calculateDaysLeft`.
   - Use `useState` to manage the `eventColor` and `amount` state.

5. Display Event Information:

   - Return a `section` element to encapsulate the component content.
   - Display breadcrumb navigation using `Link` components for Home and Events pages.

6. Event Display:

   - Create a `div` with classes for styling and a grid layout.
   - Display the event image using an `img` element with classes for styling.

7. Event Info:

   - Within a `div`, display the event title, company, and `countdown`.

8. Description:

   - Display the event description using a `p` element.

9. Colors:

   - Display available event colors using a `div` with classes for styling.
   - Map through the `colors` array and create a `button` for each color.
   - Add appropriate classes and styles for the color button based on the selected `eventColor`.

10. Amount:

    - Display a dropdown for selecting the event amount using a `div`.
...
12. Export SingleEvent Component:
    - Export the `SingleEvent` component as the default export of the module.

## Solution (22) - Single Event

- import and setup loader in the App.jsx

```js
import { useLoaderData } from 'react-router-dom';
import { calculateDaysLeft, customFetch } from '../utils';
import { Link } from 'react-router-dom';
import { useState } from 'react';
...
## Challenge (23) - Events Page (Setup)

- create following components and render in events page
  - Filters
  - EventsContainer
  - PaginationContainer
- in events page loader fetch all events

### Events.jsx

1. Import Dependencies:

   - Import `Filters`, `PaginationContainer`, and `EventsContainer` from `'../components'`.
   - Import `customFetch` from `'../utils'`.

2. Define URL and Loader Function:

   - Define a constant `url` containing the URL path to fetch events from.
   - Define a loader function that fetches event data from the defined URL.
   - Use `customFetch` to fetch the event data from the `url`.
   - Extract events and meta information from the response and return them.

3. Create Events Component:

   - Define a functional component named `Events`.
...
5. Filters Component:
   ...
   - Include the `Filters` component to allow users to apply filters to the event list.

6. EventsContainer Component:

   - Include the `EventsContainer` component to display the list of events.

7. PaginationContainer Component:

   - Include the `PaginationContainer` component to manage event list pagination.

8. Export Events Component:
   - Export the `Events` component as the default export of the module.

## Solution (23) - Events Page (Setup)
...
## Challenge (24) - Events Container

- create EventsList and render events in one column
- setup header (with total events and toggle buttons)
- toggle between EventsGrid and EventsList
...
3. Component Structure:
   ...
   - Return a `div` element containing a list of events.

4. Loop Through Events:

   - Use the `useLoaderData` hook to get the `events` data from the loader.
   - Use the `map` function to loop through each event in the `events` array.

5. Event Link:

   - For each event, create a `Link` element that links to the individual event page.
   - Use the `event.id` as the link path (`to={`/events/${event.id}`}`).
   - Add CSS classes to style the link and apply hover effects.

6. Event Image:

   - Display the event image inside an `img` element.
   - Apply appropriate classes for styling and responsive design.
   - Add hover effect to the image using CSS classes.

7. Event Details:

   - Display the event title and company using `h3` and `h4` elements.
   - Add classes for font styles and responsiveness.

8. Event eventDate:

   - Display the formatted eventDate using the `calculateDaysLeft` function.
   - Use a `p` element with appropriate classes for styling.

9. Export EventList Component:
   - Export the `EventList` component as the default export of the module.
...
1. Import Dependencies:
   ...
   - Import `useLoaderData` from `'react-router-dom'`.
   - Import `EventsGrid` and `EventsList` from their respective paths.
   - Import `useState` from `'react'`.
   - Import `BsFillGridFill` and `BsList` from `'react-icons/bs'`.

2. Create EventsContainer Component:

   - Define a functional component named `EventsContainer`.

3. Component Structure:

   - Return a `div` element containing the events container.

4. Total Events Count:

   - Use the `useLoaderData` hook to get the `meta` data from the loader.
   - Extract the `total` count of events from `meta.pagination`.
   - Use a conditional statement to handle the plural form of the word "event".
...
6. Header Section:
   ...
   - Create a `div` for the header section containing the event count and layout buttons.
   - Display the total number of events using the extracted `totalEvents` count.
...
7. Events Display:
   ...
   - Create a `div` to display the events.
   - Use conditional rendering to handle cases where no events match the search or when events are present.
   - If no events match the search, display a message.
   - If events are present and the layout is 'grid', display the `EventsGrid` component.
   - If events are present and the layout is 'list', display the `EventsList` component.

8. Export EventsContainer Component:
   - Export the `EventsContainer` component as the default export of the module.

## Solution (24) - Events Container
...
const EventsContainer = () => {
  ...
  return (
    <>
      {/* HEADER */}
      <div className='flex justify-between helpers-center mt-8 border-b border-base-300 pb-5'>
...
1. Create FormRowSelect Component:

   - Define a functional component named `FormRowSelect`.
...
6. Options:
   ...
   - Use each helper in the `list` as the `key` and `value` attributes of the `option` element.

7. Export FormRowSelect Component:
   - Export the `FormRowSelect` component as the default export of the module.
...
## Challenge (27) - Filters (eventDate)
...
4. Default Values:
   ...
   - Define default values for `step`, `maxCountdown`, and `selectedCountdown`.

5. Label and Selected eventDate Display:

   - Create a `label` element with a `for` attribute matching the `name` prop.
   - Display the capitalized label text using the `label` prop.
   - Display the selected eventDate using the `calculateDaysLeft` function.

6. Range Input:

   - Create an `input` element with `type` set to `'range'`.
   - Set the `name`, `min`, `max`, `value`, and `step` attributes.
   - Use the `selectedCountdown` state for the `value` attribute.
   - Set the `onChange` event handler to update `selectedCountdown`.

7. Min and Max eventDate Display:

   - Create a `div` element for displaying minimum and maximum eventDate values.
   - Use the `calculateDaysLeft` function for formatting and displaying max eventDate.

8. Export FormRange Component:
   - Export the `FormRange` component as the default export of the module.

## Solution (27) - Filters (eventDate )
...
const FormRange = ({ label, name, size }) => {
  const step = 1;
  const maxCountdown = 365;
  const [selectedCountdown, setSelectedCountdown] = useState(maxCountdown);
...
<FormRange label='select eventDate' name='eventDate' size='range-sm' />;
```
...
6. Styling and Layout:
   ...
   - Apply appropriate classes to style and position the form control helpers.
...
const FormCheckbox = ({ label, name, defaultValue, size }) => {
  return (
    <div className='form-control helpers-center'>
      <label htmlFor={name} className='label cursor-pointer'>
        <span className='label-text capitalize'>{label}</span>
      </label>
      <input
...
<FormCheckbox label='free mobility' name='mobility' size='checkbox-sm' />;
```
...
const Loading = () => {
  return (
    <div className='h-screen flex helpers-center justify-center'>
...
- pass params down
- use params as default values (eventDate in FormRange)
- setup reset button
...
- Render a `div` container with the class `mt-16 flex justify-end`.
  - Inside this, render another `div` with class `join`.
    - For "Prev" button:
      - Use the class `btn btn-xs sm:btn-md join-helper`.
    - For page numbers:
      - Use the class `btn btn-xs sm:btn-md border-none join-helper`.
...
const PaginationContainer = () => {
  ...
  return (
    <div className='mt-16 flex justify-end'>
      <div className='join'>
        <button
          className='btn btn-xs sm:btn-md join-helper'
          onClick={() => {
            let prevPage = page - 1;
        ...
        {pages.map((pageNumber) => {
          return (
            <button
              ...
              key={pageNumber}
              className={`btn btn-xs sm:btn-md border-none join-helper ${
...
- Create a `defaultState` object with the following properties:
  - `cartHelpers` (array)
  - `numHelpersInCart` (integer)
  - `cartTotal` (integer)
  - `mobility` (fixed value: 500)
  - `tax` (integer)
  - `orderTotal` (integer)
...
- Use the `createSlice` function to create the `cartSlice`.
  ...
  - Define several reducer functions within it:
    - `addHelper`: Logs the payload when an helper is added.
    - `clearCart`: Intended to clear all helpers from the cart.
    - `removeHelper`: Intended to remove a specific helper.
    - `editHelper`: Intended to edit a specific helper.
...
export const { addHelper, removeHelper, editHelper, clearCart } = cartSlice.actions;
...
## Challenge (33) - Add Event (SingleEventPage)

- import and dispatch addHelper action
- add helper to cart in SingleEvent page

## Solution (33) - Add Event (SingleEventPage)
...
## Challenge (34) - AddHelper Reducer

- display cartHelpers in Navbar
- setup addHelper functionality

## Building the `addHelper` Reducer

1. **Purpose**:

- This reducer updates the cart state when a event is added.

2. **Extract Event from Action**:

- Extract the `event` object from `action.payload`.

3. **Check for Existing Helper**:

- Search for the event in the `cartHelpers` array based on the `cartID`.
- If the helper already exists in the cart:
  - Increment the `amount` of the helper in the cart by the `amount` of the `event`.

4. **Add New Helper**:

- If the event does not exist in the cart:
  - Push the `event` directly into the `cartHelpers` array.

5. **Update Cart Totals**:

- Increase the `numHelpersInCart` by the `amount` of the event.
- Increase the `cartTotal` by the event's `eventDate` multiplied by its `amount`.

6. **Calculate Tax and Order Total**:

- Set `tax` as 10% of the `cartTotal`.
- Calculate the `orderTotal` as the sum of `cartTotal`, `mobility`, and `tax`.
...
- Use the `toast.success` method to display a success message: 'Helper added to cart'.

## Solution (34) - AddHelper Reducer
...
- refactor addHelper and get default state from local storage
...
- Set `tax` as 10% of the `cartTotal`.
- Determine the `orderTotal` as the sum of `cartTotal`, `mobility`, and `tax`.
...
## Challenge (36) - Clear Cart, Remove Helper and Edit Helper

- try to setup reducers for clear cart, remove helper and edit helper
...
### removeHelper Reducer:

1. **Purpose**:

- Removes a specific helper from the cart.
...
- Find the event in the `cartHelpers` array based on the `cartID`.

4. **Update Cart Helpers**:

- Filter out the event from `cartHelpers` array based on the `cartID`.

5. **Update Cart Totals**:

- Decrease the `numHelpersInCart` by the `amount` of the event.
- Decrease the `cartTotal` by the event's `eventDate` multiplied by its `amount`.
- Call the `calculateTotals` reducer to re-evaluate tax and order total.

6. **Notify User**:

- Use the `toast.error` method to display a message: 'Helper removed from cart'.

### editHelper Reducer:

1. **Purpose**:

- Modifies the amount of a specific helper in the cart.
...
- Find the helper in the `cartHelpers` array based on the `cartID`.

4. **Update Cart Totals**:

- Adjust the `numHelpersInCart` by the difference between the new amount and the helper's previous amount.
- Update the `cartTotal` based on the helper's `eventDate` and the amount difference.
- Update the helper's `amount` to the new amount.
- Call the `calculateTotals` reducer to re-evaluate tax and order total.
...
## Solution (36) - Clear Cart, Remove Helper and Edit Helper
...
The logic here is to update the total number of helpers in the cart (state.numHelpersInCart) by adjusting it based on the difference between the provided amount and the existing quantity of that helper (helper.amount). If amount is greater than helper.amount, it means that helpers are being added to the cart. If amount is less than helper.amount, it means that helpers are being removed from the cart. If they are equal, it implies no change to the quantity of that helper in the cart.

The result of the subtraction (amount - helper.amount) is then added to the current state.numHelpersInCart to reflect the new total number of helpers in the cart.

```js
state.cartTotal += helper.eventDate * (amount - helper.amount);
```

In this line, the logic is calculating the change in the total cost of the cart (state.cartTotal) based on the eventDate of the helper (helper.eventDate) and the change in the quantity of that helper (amount - helper.amount). This calculation is then added to the current state.cartTotal.

If amount is greater than helper.amount, it means more helpers are being added, so the cost of those additional helpers (difference between amount and helper.amount) is calculated by multiplying it with the eventDate of the helper. If amount is less than helper.amount, it means helpers are being removed, so the cost of those removed helpers is subtracted from the state.cartTotal. If they are equal, there is no change in the cost related to that helper.

## Challenge (37) - Setup Cart Page

- create CartHelpersList, CartTotals, CartHelper components
- export CartHelpersList, CartTotals in components/index.js
...
1. **Initialize Necessary Imports**:
   ...
   - Bring in `CartHelpersList`, `SectionTitle`, and `CartTotals` components from the `components` directory.
...
3. **Initialize State and Variables**:
   ...
   - Use the `useSelector` hook to retrieve `numHelpersInCart` from the Redux store's `cartState`.

4. **Component Logic**:

   - Check if `numHelpersInCart` is zero.
     - If true, return the `SectionTitle` component with the text 'Your cart is empty'.
     - If there are helpers in the cart, continue to display the cart details.
...
5. **Component Structure**:
   ...
   - For displaying cart helpers:
     - Use 8 of 12 columns on large screens (`lg:col-span-8`).
     - Insert the `CartHelpersList` component.
...
1. **Initialize Necessary Imports**:
   ...
   - Bring in `calculateDaysLeft` function from the `utils` directory for eventDate formatting.
...
3. **Retrieve State from Redux**:
   ...
   - Use the `useSelector` hook to get `cartTotal`, `mobility`, `tax`, and `orderTotal` from the Redux store's `cartState`.
...
4. **Component Structure**:
   ...
   - Display Shipping charges:
     - Similar to the Subtotal, but the text reads "Shipping" and the value is the formatted `mobility`.
   - Display Tax:
...
import calculateDaysLeft from '../utils/axios';
const CartTotals = () => {
  const { cartTotal, mobility, tax, orderTotal } = useSelector(
    ...
    <div className='card bg-base-200'>
      <div className='card-body'>
        ...
        <p className='flex justify-between text-xs border-b border-base-300 pb-2'>
          <span>Subtotal</span>
          <span className='font-medium'>{calculateDaysLeft(cartTotal)}</span>
        </p>
        {/* SHIPPING */}
        <p className='flex justify-between text-xs border-b border-base-300 pb-2'>
          <span>Shipping</span>
          <span className='font-medium'>{calculateDaysLeft(mobility)}</span>
        </p>
        {/* Tax */}
        <p className='flex justify-between text-xs border-b border-base-300 pb-2'>
          <span>Tax</span>
          <span className='font-medium'>{calculateDaysLeft(tax)}</span>
        </p>
        {/* Total */}
        <p className='mt-4 flex justify-between text-sm  pb-2'>
          <span className='font-bold'>Order Total</span>
          <span className='font-bold'>{calculateDaysLeft(orderTotal)}</span>
        </p>
      </div>
    </div>
  );
...
## Challenge (39) - Cart Helpers List

- iterate over cartHelpers and return CartHelper
- in CartHelper display values and implement remove,edit functionality
...
1. **Initialize Necessary Imports**:
   ...
   - Bring in `CartHelper` component for helper rendering.

2. **Create CartHelpersList Component**:

   - Define a functional component named `CartHelpersList`.

3. **Retrieve State from Redux**:

   - Use the `useSelector` hook to get `cartHelpers` from the Redux store's `cartState`.

4. **Component Structure**:

   - Create a wrapping `div`.
   - Use the `map` method on `cartHelpers` to loop through each helper.
   - For each `helper`, return a `CartHelper` component:
     - Pass `helper.cartID` as the `key` prop for React's list rendering optimization.
     - Pass the entire `helper` object as the `cartHelper` prop to the `CartHelper` component.

5. **Export CartHelpersList Component**:

   - Export the `CartHelpersList` component as the default export of the module.

### CartHelper.js

1. **Initialize Necessary Imports**:

   - Import utility functions `calculateDaysLeft` and `generateAmountOptions` from the `../utils` directory.
   - Import `removeHelper` and `editHelper` from the Redux slice named `cartSlice`.
   - Bring in `useDispatch` from `react-redux` for dispatching actions to the Redux store.

2. **Create CartHelper Component**:

   - Define a functional component named `CartHelper` that accepts a `cartHelper` prop.
...
4. **Functions for Event Handlers**:
   ...
   - Define `removeHelperFromTheCart` function:
     - Dispatch the `removeHelper` action, passing the `cartID` from the cart helper.
   - Define `handleAmount` function:
     - Dispatch the `editHelper` action, updating the `amount` for the cart helper identified by `cartID`.

5. **Destructure cartHelper Prop**:

   - Extract necessary fields from `cartHelper` including `cartID`, `title`, `eventDate`, `image`, `amount`, `company`, and `eventColor`.

6. **Component Structure**:

   - Create an `article` element wrapping the entire cart helper.
     - Embed the event image using an `img` element.
     - Display event information using a `div`:
       - Show the event `title`, `company`, and `color` using corresponding elements.
     - Render the event amount using a `div` containing a dropdown `select`.
     - Provide a `button` to remove the helper from the cart.
     - Display the event eventDate using a `p` element.

7. **Export CartHelper Component**:

   - Export the `CartHelper` component as the default export of the module.

## Solution (39) - Cart Helpers List
...
```js
import { calculateDaysLeft, generateAmountOptions } from '../utils';
import { removeHelper, editHelper } from '../features/cart/cartSlice';
import { useDispatch } from 'react-redux';
const CartHelper = ({ cartHelper }) => {
  const dispatch = useDispatch();

  const removeHelperFromTheCart = () => {
    dispatch(removeHelper({ cartID }));
  };
  const handleAmount = (e) => {
    dispatch(editHelper({ cartID, amount: parseInt(e.target.value) }));
  };

  const { cartID, title, eventDate, image, amount, company, eventColor } =
    cartHelper;
  ...
  return (
    ...
    >
      ...
      <div className='sm:ml-16 sm:w-48'>
        ...
        {/* COLOR */}
        <p className='mt-4 text-sm capitalize flex helpers-center gap-x-2'>
          color :
          <span
      ...
      <div className='sm:ml-12'>
        ...
        <button
          ...
          onClick={removeHelperFromTheCart}
        >
          remove
        </button>
      </div>
      ...
      {/* eventDate */}
      <p className='font-medium sm:ml-auto'>{calculateDaysLeft(eventDate )}</p>
    </article>
  );
};
...
- create features/users/userSlice.js
- Import Dependencies:
...
- Define Reducer Functions:
  ...
  - Create the `login` reducer function with the signature `(state, action) => {}`.
...
const Header = () => {
  ...
  return (
    <header className=' bg-neutral py-2 text-neutral-content '>
      <div className='align-element flex justify-center sm:justify-end '>
        {user ? (
          <div className='flex gap-x-2 sm:gap-x-8 helpers-center'>
...
- Use a `try` block to handle the login process:
  ...
  - If the request is successful:
    - Dispatch the `login` action with the response data using `store.dispatch`.
    - Display a success toast message using `toast.success`.
...
- Assign the response from the `customFetch.post` call to the `response` variable.- If the request is successful, dispatch an action (e.g., `login`) with the `response.data`.
...
- Create the `Checkout` component:
  ...
  - If the `cartTotal` is not empty:
    ...
    - Render a `<div>` element with the class name 'mt-8 grid gap-8 md:grid-cols-2 helpers-start'.
...
const Checkout = () => {
  const cartHelpers = useSelector((state) => state.cartState.cartTotal);
  ...
  return (
    <>
      ...
      <div className='mt-8 grid gap-8  md:grid-cols-2 helpers-start'>
        <CheckoutForm />
        <CartTotals />
      </div>
    </>
...
- Create an `action` function:
  ...
  - Inside the async function:
    ...
    - Get the `user` from the Redux store using `store.getState().userState.user`.
    - Get the `cartHelpers`, `orderTotal`, and `numHelpersInCart` from the Redux store using `store.getState().cartState`.
...
- Create a `CheckoutForm` component:
  ...
  - Inside the component:
    ...
    - Display a heading for the mobility information.
...
import { customFetch, calculateDaysLeft } from '../utils';
import { toast } from 'react-toastify';
import { clearCart } from '../features/cart/cartSlice';
...
export const action =
  ...
  async ({ request }) => {
    ...
    const user = store.getState().userState.user;
    const { cartHelpers, orderTotal, numHelpersInCart } =
    ...
    const info = {
      ...
      orderTotal: calculateDaysLeft(orderTotal),
      cartHelpers,
      numHelpersInCart,
    };
    try {
...
- Create the `OrdersList` component:
  ...
  - Inside the component:
    ...
    - Return the orders list interface:
      ...
      - Define table headings: Name, Address, Events, Cost, and Date.
      ...
      <div className='overflow-x-auto '>
        <table className='table table-zebra'>
          ...
          <tbody>
            {orders.map((order) => {
              const id = order.id;
              const { name, address, numHelpersInCart, orderTotal, createdAt } =
                order.attributes;
...
const ComplexPaginationContainer = () => {
  ...
  const renderPageButtons = () => {
    ...
    if (page > 2) {
      pageButtons.push(
        <button className='join-helper btn btn-xs sm:btn-md' key='dots-1'>
          ...
        </button>
      );
    }
    ...
    if (page < pageCount - 1) {
      pageButtons.push(
        <button className='join-helper btn btn-xs sm:btn-md' key='dots-2'>
          ...
        </button>
      );
    }
...
## Challenge (57) - Single Event

- setup react query and invoke in loader

## Solution (57) - Single Event
...
const singleEventQuery = (id) => {
  return {
    ...
    queryFn: () => customFetch.get(`/events/${id}`),
  };
};
...
## Challenge (58) - All Events

- setup react query and invoke in loader

## Solution (58) - All Events
